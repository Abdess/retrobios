...
void main(){
 ...
 bool upscale = (1.0/outX) > (1.6 * (1.0/resX));
 vec3 res = texture2D(uTex, v_texcoord0.xy).xyz;
 ...
 if (upscale) {
	...
	vec2 pS  = 1.0 / (vec2(1.0/outX,1.0/outY));
	vec2 fp  = fract(v_texcoord0.xy*pS.xy);
	vec2 TexCoord_0 = v_texcoord0.xy-fp*vec2(1.0/resX,1.0/resY);
	vec2 dx  = vec2(1.0/outX,0.0);
	vec2 dy  = vec2(0.0,1.0/outY);
	vec2 y2  = dy + dy; vec2 x2  = dx + dx;


	vec3 A  = texture2D(uTex, TexCoord_0 -dx -dy	).xyz;
	vec3 B  = texture2D(uTex, TexCoord_0	 -dy	).xyz;
	vec3 C  = texture2D(uTex, TexCoord_0 +dx -dy	).xyz;
	vec3 D  = texture2D(uTex, TexCoord_0 -dx		).xyz;
	vec3 E  = texture2D(uTex, TexCoord_0			).xyz;
	vec3 F  = texture2D(uTex, TexCoord_0 +dx		).xyz;
	vec3 G  = texture2D(uTex, TexCoord_0 -dx +dy	).xyz;
	vec3 H  = texture2D(uTex, TexCoord_0	 +dy	).xyz;
	vec3 I  = texture2D(uTex, TexCoord_0 +dx +dy	).xyz;
	vec3 A1 = texture2D(uTex, TexCoord_0	 -dx -y2).xyz;
	vec3 C1 = texture2D(uTex, TexCoord_0	 +dx -y2).xyz;
	vec3 A0 = texture2D(uTex, TexCoord_0 -x2	 -dy).xyz;
	vec3 G0 = texture2D(uTex, TexCoord_0 -x2	 +dy).xyz;
	vec3 C4 = texture2D(uTex, TexCoord_0 +x2	 -dy).xyz;
	vec3 I4 = texture2D(uTex, TexCoord_0 +x2	 +dy).xyz;
	vec3 G5 = texture2D(uTex, TexCoord_0	 -dx +y2).xyz;
	vec3 I5 = texture2D(uTex, TexCoord_0	 +dx +y2).xyz;
	vec3 B1 = texture2D(uTex, TexCoord_0		 -y2).xyz;
	vec3 D0 = texture2D(uTex, TexCoord_0 -x2		).xyz;
	vec3 H5 = texture2D(uTex, TexCoord_0		 +y2).xyz;
	vec3 F4 = texture2D(uTex, TexCoord_0 +x2		).xyz;
